...
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/front-end/bootstrap-4.5.3-dist/css/storePickupNoAndNoCall.css" />
</head>
<body>
	<div class="container">
					...
					<div class="row right-li">
						...
						<div class="col-2">
							...
							<c:forEach var="quePeriodVO" items="${quePeriodVO}">
								<c:choose>
									<c:when
										test="${quePeriodVO.queueperiodid ==1 && queTableVO.queuetableid ==1}">
										<div class="no_current">${quePeriodVO.queuenocurrent }</div>
									</c:when>
									<c:when
										test="${quePeriodVO.queueperiodid ==2 && queTableVO.queuetableid ==2}">
										<div class="no_current">${quePeriodVO.queuenocurrent }</div>
									</c:when>
									<c:when
										test="${quePeriodVO.queueperiodid ==3 && queTableVO.queuetableid ==3}">
										<div class="no_current">${quePeriodVO.queuenocurrent }</div>
									</c:when>
									<c:when
										test="${quePeriodVO.queueperiodid ==4 && queTableVO.queuetableid ==4}">
										<div class="no_current">${quePeriodVO.queuenocurrent }</div>
									</c:when>
								</c:choose>
							</c:forEach>
