...
public class QueLineServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("insert".equals(action)) {
			...
			String url = "/front-end/queue/queueLine/listAllQueLine.jsp";
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {
				...
				String url = "/front-end/queue/queueLine/update_queLine_input.jsp";

				RequestDispatcher successView = req.getRequestDispatcher(url);

				successView.forward(req, res);
			} catch (Exception e) {
				errorMsgs.add("?Ñ°Ê≥ïÂ?ñÂ?óË?Å‰øÆ?îπ??ÑË?áÊ??:" + e.getMessage());
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/queue/queueLine/listAllQueLine.jsp");
		...
		if ("list_One_Store".equals(action)) {
			...
			try {
				...
				req.setAttribute("queLineVO", queLineVO);
				String url = "/front-end/queue/queueLine/listOneStore.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
			} catch (Exception e) {
				errorMsgs.add("?Ñ°Ê≥ïÂ?ñÂ?óË?Å‰øÆ?îπ??ÑË?áÊ??:" + e.getMessage());
